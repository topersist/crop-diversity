...
for gcm = 1:size(v_data_hold_future,4)
    for rcp = 1:size(v_data_hold_future,3)
        if (gcm==8) && (rcp==4)
            v_temp = NaN(size(v_data_distance(:,1,rcp,gcm)));
        else
...
% % coordinate y), RCP (2.6, 4.5, 7.0, 8.5), GCMs (8 different GCMs)
% 
...
% future - SSP2_4.5
geotiffNameRCP45 = sprintf('holdridge_data/holdridge_5min_RCP45_year%s_%s.tif', futureYear,date );
...
temp(temp == 0) = NaN;
geotiffwrite(geotiffNameRCP45,int16(temp),R_5arcmin);


% future - SSP3_7.0
geotiffNameRCP70 = sprintf('holdridge_data/holdridge_5min_RCP70_year%s_%s.tif', futureYear,date );
...
temp(temp == 0) = NaN;
geotiffwrite(geotiffNameRCP70,int16(temp),R_5arcmin);
